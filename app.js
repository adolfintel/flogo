    ...
    if (variables[v.flogo_variable].isArray) {
        ...
        if (v.flogo_val.vis.arrayViewer.open) {
            ...
            for (let i = 0; i < v.flogo_val.vis.arrayViewer.arrContents.length; i++) {
                if (typeof v.flogo_val.vis.arrayViewer.arrContents[i].flogo_value === "undefined" || variables[v.flogo_variable].value[i] !== v.flogo_val.vis.arrayViewer.arrContents[i].flogo_value) {
                    needRefresh = true
                    break
                }
            }
...
function registerWindowEvents(w, canResize = false) {
        ...
        w.onmousedown = e => {
            ...
            if (resize !== "") {
                ...
                document.onmousemove = e => {
                    ...
                    const x = _extractCoordFromEvent(e, "clientX") - currentBounds.x,
                        y = _extractCoordFromEvent(e, "clientY") - currentBounds.y
    ...
    bar.ontouchstart = bar.onmousedown = e => {
        ...
        document.ontouchmove = document.onmousemove = e => {
            e.preventDefault()
            const x = _extractCoordFromEvent(e, "clientX") - offX,
                y = _extractCoordFromEvent(e, "clientY") - offY
            w.style.left = x + "px"
            w.style.top = y + "px"
            limitWindowWithinBounds(w)
        }
...
function initApp() {
    ...
    window.addEventListener("load", () => {
        document.fonts.ready.then(() => {
            ...
            resetConsole()
            clearTurtle()
            const turtleArea = document.getElementById("turtleArea")
            registerWindowEvents(turtleArea, true)
